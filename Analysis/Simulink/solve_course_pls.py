...
def distance(p1, p2):
    return math.sqrt((p2[0] - p2[0])**2 + (p1[1] - p2[1])**2 + 0.0)
...
def dynamic_programming():
    ...
    distances = [[distance(nodes_list[i], nodes_list[j])
                  for i in range(ntargets)] for j in range(ntargets)]
    subset_costs = {}

    for end in range(ntargets):
        subset_costs[(tuple([end]), end)] = (
            distance(nodes_list[end], (0, 0)), -1, end)
    ...
    while len(queue) != 0:
        ...
        for j in (-1, 0, 1):
            for i in (-1, 0, 1):
                ...
                y0, x0 = y + j, x + i
                n = (y0, x0)
                if (-1 in n) or (N in n) or (n in path):
                    continue
                if not in_obstacle_border(y0, x0):
                    ...
                    score = d * (0.1 + 0.9 * obstacle_heatmap[y][x])  # +...

                    # Insertion sort
                    for k in range(len(queue) + 1):
                        if (k == len(queue)) or (queue[k][0] >= score):
                            queue.insert(
                                k, (score, list(path) + [n], length + distance(current, n), hops + 1))
                            count += 1
                            break
...
def find_result(filename):
    ...
    draw_results = False  # If False, no matplotlib or a computer moniter are needed
    debug = False
    ...
    for y in range(N):
        for x in range(N):
            ...
            if (y, x) not in obsta_list:
                if obstacle_heatmap[y][x] > max_:
                    max_ = obstacle_heatmap[y][x]
                if obstacle_heatmap[y][x] < min_:
                    min_ = obstacle_heatmap[y][x]
    ...
    for y in range(N):
        for x in range(N):
            if (y, x) in obsta_list:
                ...
                obstacle_heatmap[y][x] = (
                    obstacle_heatmap[y][x] - min_) / (max_ - min_ + 0.000001)
    ...
    if debug:
        print("%d/%d" % (len(min_path) - 1, len(min_path) - 1))

    # Remove unnecessary points
    comp_path = [(0, 0)]
    for n in range(1, len(quantized_path) - 1):
        prev, cur, next = quantized_path[n -
                                         1], quantized_path[n], quantized_path[n + 1]
    ...
    if draw_results:
        ...
        plt.ylabel('y')
        plt.title('')
        plt.plot(np.array(min_path).T[0], np.array(
            min_path).T[1], 'r--', label="best path (TSP)")
        plt.plot(np.array(comp_path).T[0], np.array(
            comp_path).T[1], "b-", label="A* avoid obstacles")
        plt.plot(np.array(nodes_list).T[0], np.array(
            nodes_list).T[1], "bo", mfc='none', label="targets")
        plt.plot(np.array(obsta_list).T[0], np.array(
            obsta_list).T[1], "rx", label="obstacles")
        plt.grid(True)
        ...
        for y in range(N):
            for x in range(N):
                ax.add_patch(patches.Rectangle((y - 0.5, x - 0.5), 1, 1, fill=True,
                                               linewidth=0, facecolor="#106d00", alpha=obstacle_heatmap[y][x]))

        # White boxes for targets and obstacles
        for n in nodes_list + obsta_list:
            ax.add_patch(patches.Rectangle(
                (n[0] - 1, n[1] - 1), 2, 2, fill=True, facecolor="#ffffff", edgecolor="#000000"))
