...
def dynamic_programming():	
	...
	# Trace solution
	solution, nodes = [], range(len(nodes_list))
	while (min_sol[1]!=-1):
		...
		for j in (-1,0,1):
			for i in (-1,0,1):
				...
				if not in_obstacle_border(y0,x0):	
					...
					# (better but more manoeuvres for crane) / more waypoints
					#d = distance((y0,x0), goal)
					#if 0 not in (j,i):
					#	d = distance((y+j/sqrt2,x+i/sqrt2), goal)
					
					# HEURISTICS (subject to fine-tune)	
					#score = d * ( 0.1+ 0.9* obstacle_heatmap[y][x]) +40*obstacle_heatmap[y][x]#+ 0.1*length 	
					score = d * ( 0.1+ 0.9* obstacle_heatmap[y][x]) +4*obstacle_heatmap[y][x]#+ 0.1*length
					# Insertion sort
